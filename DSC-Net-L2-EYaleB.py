...
class ConvAE(object):
    def __init__(self, n_input, kernel_size, n_hidden, reg_constant1 = 1.0, re_constant2 = 1.0, batch_size = 200, reg = None, \
        ...
        self.Coef = Coef # - tf.diag(tf.diag_part(Coef))       
        ...
        #weights_value = weights.values()         
        #self.reg_losses = 0.0
        #for _, value in enumerate(weights_value):
        #    self.reg_losses = self.reg_losses + tf.nn.l2_loss(value)
        ...
        #self.compute_grad = tf.train.AdamOptimizer(learning_rate = self.learning_rate).compute_gradients(self.loss, [v for v in tf.trainable_variables() if (v.name.startswith("Coef"))])
...
def build_laplacian(C):
    ...
    W = np.diag(1.0/W)
    L = W.dot(C)
    #L = L.dot(W) # normalized Laplacian matrix
    return L
...
def test_face(Img, Label, CAE, num_class):       
    ...
    alpha = 0.48 - num_class/100.0 #max(0.4 - (num_class-1)/10 * 0.1, 0.1)#
    print alpha
    ...
    for i in range(0,39-num_class): 
        ...
        #face_10_subjs = face_10_subjs/face_10_subjs.max()
        ...
        while epoch < max_step:
            ...
            if (epoch % display_step == 0) or (epoch == max_step):
                print "epoch: %.1d" % epoch, "cost: %.8f" % (cost/float(batch_size))                
                C = thrC(Coef,alpha)                                          
                y_x, _ = post_proC(C, label_10_subjs.max(), 10, 3.5)                  
                missrate_x = err_rate(label_10_subjs, y_x)                
                acc_x = 1 - missrate_x 
                print "experiment: %d" % i, "our accuracy: %.4f" % acc_x
        acc_.append(acc_x) 
        """
        C_show = np.zeros((640,640))
        for ii in range(10):
            C_show[(ii*64):(ii*64+64),(ii*64):(ii*64+64)] = Coef[(ii*64):(ii*64+64),(ii*64):(ii*64+64)]
        C_show = C_show - np.diag(np.diag(C_show))
        C_show = C_show/C_show.max()
        plt.figure(figsize=(8,8))    
        plt.imshow(np.abs(C_show))
        #plt.title("Our Affinity Matrix")
        plt.colorbar()
        plt.tight_layout()
...
if __name__ == '__main__':
    ...
    while iter_loop < len(all_subjects):
        ...
        reg1 = 1.0
        reg2 = 2.0#1.0 * 10 ** (num_class / 10.0 - 3.0) 
        #model_path = '/home/pan/workspace-eclipse/deep-subspace-clustering/conv_3_l1_yaleb/pretrain/model.ckpt'
        #restore_path = '/home/pan/workspace-eclipse/deep-subspace-clustering/conv_3_l1_yaleb/pretrain/model.ckpt' 
        # model_path = '/home/pan/workspace-eclipse/deep-subspace-clustering/models_face/model-102030-c.ckpt'
        # restore_path = '/home/pan/workspace-eclipse/deep-subspace-clustering/models_face/model-102030-c.ckpt'  
        model_path = '/home/pan/workspace-eclipse/deep-subspace-clustering/models_face/model-302010-48x42-yaleb.ckpt' 
        restore_path = '/home/pan/workspace-eclipse/deep-subspace-clustering/models_face/model-302010-48x42-yaleb.ckpt' 
